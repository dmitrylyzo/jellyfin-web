...
/* eslint-disable indent */
    ...
    function renderViews(page, user, result) {
        ...
        folderHtml += result.map(i => {
            ...
            currentHtml += `<span>${htmlescape(i.Name)}</span>`;
            currentHtml += '</label>';
    ...
    function getLandingScreenOptionsHtml(type, userValue) {
        return getLandingScreenOptions(type).map(o => {
            ...
            return `<option value="${optionValue}"${selectedHtml}>${htmlescape(o.name)}</option>`;
        }).join('');
    }
    ...
    function renderViewOrder(context, user, result) {
        ...
        html += result.Items.map((view) => {
            ...
            currentHtml += '<div>';
            currentHtml += htmlescape(view.Name);
            currentHtml += '</div>';
