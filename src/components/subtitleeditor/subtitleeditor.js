...
function fillSubtitleList(context, item) {
    ...
    if (subs.length) {
        ...
        html += subs.map(function (s) {
            ...
            itemHtml += '<div>';
            itemHtml += htmlescape(s.DisplayTitle || '');
            itemHtml += '</div>';

            if (s.Path) {
                itemHtml += '<div class="secondary listItemBodyText">' + htmlescape(s.Path) + '</div>';
            }
...
function renderSearchResults(context, results) {
    ...
    for (let i = 0, length = results.length; i < length; i++) {
        ...
        html += '<div>' + htmlescape(result.Name) + '</div>';
        ...
        if (result.Comment) {
            html += '<div class="secondary listItemBodyText">' + htmlescape(result.Comment) + '</div>';
        }
