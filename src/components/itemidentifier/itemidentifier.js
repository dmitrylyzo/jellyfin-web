...
import template from './itemidentifier.template.html';
    ...
    function showIdentifyOptions(page, identifyResult) {
        ...
        const lines = [];
        lines.push(htmlescape(identifyResult.Name));
    ...
    function getSearchResultHtml(result, index) {
        ...
        if (result.ImageUrl) {
            ...
            html += `<div class="cardImageContainer coveredImage defaultCardBackground defaultCardBackground1"><div class="cardText cardCenteredText">${htmlescape(result.Name)}</div></div>`;
        }
        html += '</div>';
        html += '</div>';
    ...
    function showIdentificationForm(page, item) {
        ...
        apiClient.getJSON(apiClient.getUrl(`Items/${item.Id}/ExternalIdInfos`)).then(idList => {
            ...
            for (let i = 0, length = idList.length; i < length; i++) {
                ...
                const idLabel = globalize.translate('LabelDynamicExternalId', htmlescape(fullName));
