...
function updateNowPlayingInfo(context, state, serverId) {
    ...
    if (item) {
        ...
        if (item.Type == 'Audio' || item.MediaStreams[0].Type == 'Audio') {
            const songName = htmlescape(item.Name);
            let artistsSeries = '';
            let albumName = '';
            if (item.Artists != null) {
                if (item.ArtistItems != null) {
                    for (const artist of item.ArtistItems) {
                        const artistName = htmlescape(artist.Name);
                    ...
                    for (const artist of item.Artists) {
                        artistsSeries += `<a>${htmlescape(artist)}</a>`;
            ...
            if (item.Album != null) {
                albumName = '<a class="button-link emby-button" is="emby-linkbutton" href="#!/details?id=' + item.AlbumId + `&serverId=${nowPlayingServerId}">` + htmlescape(item.Album) + '</a>';
            }
            ...
            if (item.SeasonName != null) {
                ...
                context.querySelector('.nowPlayingSeason').innerHTML = '<a class="button-link emby-button" is="emby-linkbutton" href="#!/details?id=' + item.SeasonId + `&serverId=${nowPlayingServerId}">${htmlescape(seasonName)}</a>`;
            }
            if (item.SeriesName != null) {
                const seriesName = item.SeriesName;
                if (item.SeriesId != null) {
                    context.querySelector('.nowPlayingSerie').innerHTML = '<a class="button-link emby-button" is="emby-linkbutton" href="#!/details?id=' + item.SeriesId + `&serverId=${nowPlayingServerId}">${htmlescape(seriesName)}</a>`;
                } else {
