...
export function show(options) {
    ...
    if (options.title) {
        html += '<h1 class="actionSheetTitle">' + htmlescape(options.title) + '</h1>';
    }
    if (options.text) {
        html += '<p class="actionSheetText">' + htmlescape(options.text) + '</p>';
    }
    ...
    for (let i = 0; i < options.items.length; i++) {
        ...
        html += htmlescape(item.name || item.textContent || item.innerText);
        html += '</div>';

        if (item.secondaryText) {
            html += `<div class="listItemBodyText secondary">${htmlescape(item.secondaryText)}</div>`;
        }

        html += '</div>';

        if (item.asideText) {
            html += `<div class="listItemAside actionSheetItemAsideText">${htmlescape(item.asideText)}</div>`;
        }
