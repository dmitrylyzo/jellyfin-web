...
function renderTrackSelections(page, instance, item, forceReload) {
    ...
    select.innerHTML = mediaSources.map(function (v) {
        ...
        return '<option value="' + v.Id + '"' + selected + '>' + htmlescape(v.Name) + '</option>';
    }).join('');
...
function getArtistLinksHtml(artists, serverId, context) {
    ...
    for (const artist of artists) {
        ...
        html.push('<a style="color:inherit;" class="button-link" is="emby-linkbutton" href="' + href + '">' + htmlescape(artist.Name) + '</a>');
    }
...
function renderName(item, container, context) {
    ...
    if (item.AlbumArtists) {
        ...
        parentNameHtml.push(`<a style="color:inherit;" class="button-link itemAction" is="emby-linkbutton" href="#" data-action="link" data-id="${item.SeriesId}" data-serverid="${item.ServerId}" data-type="Series" data-isfolder="true">${htmlescape(item.SeriesName)}</a>`);
    } else if (item.IsSeries || item.EpisodeTitle) {
        parentNameHtml.push(htmlescape(item.Name));
    }

    if (item.SeriesName && item.Type === 'Season') {
        parentNameHtml.push(`<a style="color:inherit;" class="button-link itemAction" is="emby-linkbutton" href="#" data-action="link" data-id="${item.SeriesId}" data-serverid="${item.ServerId}" data-type="Series" data-isfolder="true">${htmlescape(item.SeriesName)}</a>`);
    } else if (item.ParentIndexNumber != null && item.Type === 'Episode') {
        parentNameHtml.push(`<a style="color:inherit;" class="button-link itemAction" is="emby-linkbutton" href="#" data-action="link" data-id="${item.SeasonId}" data-serverid="${item.ServerId}" data-type="Season" data-isfolder="true">${htmlescape(item.SeasonName)}</a>`);
    } else if (item.ParentIndexNumber != null && item.IsSeries) {
        parentNameHtml.push(htmlescape(item.SeasonName) || 'S' + item.ParentIndexNumber);
    } else if (item.Album && item.AlbumId && (item.Type === 'MusicVideo' || item.Type === 'Audio')) {
        parentNameHtml.push(`<a style="color:inherit;" class="button-link itemAction" is="emby-linkbutton" href="#" data-action="link" data-id="${item.AlbumId}" data-serverid="${item.ServerId}" data-type="MusicAlbum" data-isfolder="true">${htmlescape(item.Album)}</a>`);
    } else if (item.Album) {
        parentNameHtml.push(htmlescape(item.Album));
    }
    ...
    const name = htmlescape(itemHelper.getDisplayName(item, {
    ...
    if (item.OriginalTitle && item.OriginalTitle != item.Name) {
        html += '<h4 class="itemName infoText originalTitle">' + htmlescape(item.OriginalTitle) + '</h4>';
    }
...
function renderSeriesAirTime(page, item, isStatic) {
    ...
    if (item.Studios.length) {
        if (isStatic) {
            html += ' on ' + htmlescape(item.Studios[0].Name);
        } else {
            ...
            html += ' on <a class="textlink button-link" is="emby-linkbutton" href="' + href + '">' + htmlescape(item.Studios[0].Name) + '</a>';
        }
    }
    if (html) {
