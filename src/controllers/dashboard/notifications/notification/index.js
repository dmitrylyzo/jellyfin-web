...
function fillItems(elem, items, cssClass, idPrefix, currentList, isEnabledList) {
    ...
    html += items.map(function (u) {
        ...
        return '<label><input is="emby-checkbox" class="' + cssClass + '" type="checkbox" data-itemid="' + u.Id + '"' + checkedHtml + '/><span>' + htmlescape(u.Name) + '</span></label>';
    }).join('');
    html += '</div>';
...
function reload(page) {
    ...
    Promise.all([promise1, promise2, promise3, promise4]).then(function (responses) {
        ...
        $('.notificationType', page).html(htmlescape(typeInfo.Name) || 'Unknown Notification');
