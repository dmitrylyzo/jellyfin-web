...
/* eslint-disable indent */
    ...
    function renderActiveConnections(view, sessions) {
        ...
        for (let i = 0, length = sessions.length; i < length; i++) {
            ...
            if (elem) {
                ...
                html += '<div class="sessionDeviceName">' + htmlescape(session.DeviceName) + '</div>';
                html += '<div class="sessionAppSecondaryText">' + htmlescape(DashboardPage.getAppSecondaryText(session)) + '</div>';
                html += '</div>';
                html += '</div>';
                ...
                html += '</div>';
                html += '<div class="sessionNowPlayingTime">' + htmlescape(DashboardPage.getSessionNowPlayingTime(session)) + '</div>';
                html += '</div>';
    ...
    window.DashboardPage = {
        ...
        getNowPlayingName: function (session) {
            ...
            let topText = htmlescape(itemHelper.getDisplayName(nowPlayingItem));
            let bottomText = '';

            if (nowPlayingItem.Artists && nowPlayingItem.Artists.length) {
                bottomText = topText;
                topText = htmlescape(nowPlayingItem.Artists[0]);
            } else {
                if (nowPlayingItem.SeriesName || nowPlayingItem.Album) {
                    bottomText = topText;
                    topText = htmlescape(nowPlayingItem.SeriesName || nowPlayingItem.Album);
